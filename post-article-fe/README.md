...
│   │   ├── posts/
│   │   │   ├── page.tsx          # All Posts: Published / Draft / Trashed tabs + search
│   │   │   ├── new/page.tsx      # Create Article
│   │   │   └── [id]/edit/page.tsx# Edit Article (+ Move to Trash)
│   │   └── preview/
│   │       ├── page.tsx          # Published blog list (10/page + search)
...
│   │   ├── SearchBox.tsx         # debounced search input (reusable)
│   │   ├── StatusBadge.tsx       # publish/draft/trash pill
│   │   ├── IconButton.tsx        # icon-only action button (icon + title tooltip)
│   │   ├── icons.tsx             # inline SVG icons (edit, trash, publish, draft…)
│   │   └── feedback.tsx          # Alert / Loading / EmptyState / Toast / skeletons
│   ├── lib/
...
```ts
type ArticleStatus = "publish" | "draft" | "trash";
...
| `/posts/[id]/edit` | Same form prefilled; `PUT /article/:id` on save; **Move to Trash** button included. |
| `/preview` (Preview) | Blog grid of **publish-status** articles only, **10 per page** (BE `limit/offset` + `?status=publish`, FE prev/next pager) + debounced **search**. |
| `/preview/[id]` | Full blog detail with category, status badge, date. |

## UI/UX notes

- Light theme only, no UI framework (zero extra runtime deps) — fast and lightweight.
- Skeleton shimmer placeholders for tables/cards instead of blank loaders.
- Status/trash actions are **optimistic** (row updates instantly, rolls back on error) with toast confirmation.
- Search is **FE-only with debounce** (250ms): pulls a wider window (`limit=100`) per tab/filter, then matches title/category client-side. No BE changes needed.
...
- `GET /article/:limit/:offset?status=publish|draft|trash` — status filter for tabs + preview
- `DELETE /article/:id` — **soft-delete** (status → `trash`), feeds the Trash tab
